            ...
            {filtered.map(p => (
              <div key={p.id} className="glass p-6 rounded-[2rem] border-white/40 hover:shadow-xl transition group">
                ...
                <div className="flex gap-3">
                  <Link 
                    to={`/chat/${p.id}`} 
                    onClick={() => sessionStorage.setItem(`peer_${p.id}`, JSON.stringify(p))}
                    className="flex-1 py-3 rounded-xl bg-teal-700 text-white text-[11px] font-black uppercase tracking-widest text-center shadow-lg shadow-teal-700/20 hover:scale-105 transition active:scale-95">Chat</Link>
